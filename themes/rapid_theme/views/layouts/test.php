...
<script>
    ...
    $(function () {
        ...
        $(".uk-close").on('click', function () {
            try {
                $('.uk-modal-dialog').removeClass('uk-modal-dialog-large');
    ...
    // Create, Update, View url
    $(document).on('click', '.view, .update,.create', function (event) {
        event.preventDefault();
        var url = $(this).attr('href');
        var title = $(this).data("title");
        var popupsize = $(this).data("popupsize");
        if (popupsize == "L")
            $(".uk-modal-dialog").addClass("uk-modal-dialog-large");
        else
            $(".uk-modal-dialog").removeClass("uk-modal-dialog-large");

        $('.uk-modal-title').html(title);
        $.ajax({
            url: url,
            success: function (result) {
                $('.uk-model_content').html(result);
                $('.popup').trigger('click');
                altair_md.init();
                altair_forms.init();
            },
    ...
    // Delete url
    $(document).on('click', '.delete, .inactive', function (e) {
        e.preventDefault();
        var url = $(this).attr('href');
        var grid_id = $(this).closest('.grid-view').attr('id');
        var deletemsg = $(this).data('deletemsg');
        UIkit.modal.confirm("Are you sure, you want to " + deletemsg + "?", function () {
            $.ajax({
                url: url,
                type: 'POST',
                success: function (result) {
                    ...
                    switch (result.status) {
                        case 'S':
                        {
                            show_flash_msg(result.msg, "S");
                            Refresh.CgridRefresh(grid_id);
                            break;
                        }
                        case 'E':
                        {
                            show_flash_msg(result.msg, "E");
                            break;
                        }
                        default:
                        {
    ...
    function show_flash_msg(msg, status, time) {
        time = (time == undefined) ? 8000 : time;
        $("#div_flash_msg").removeClass();
        $("#div_flash_msg").addClass('uk-notify-message');
        $(".uk-notify").stop();
        $(".uk-notify").show();
        if (status == "S")
            $("#div_flash_msg").addClass("uk-notify-message-success");
        else if (status == "E")
            $("#div_flash_msg").addClass("uk-notify-message-danger");
        else
            $("#div_flash_msg").addClass("uk-notify-message-info");
        $(".uk-notify-top-right").fadeOut(time);
        $("#alert_msgg").html(msg);
    }
    ...
    function Save_datas(form, data, hasError) {
        if (hasError == false) {
            var grid_id = $(document).find('.grid-view').attr('id');
            $.ajax({
                type: 'POST',
                ...
                success: function (result) {
                    ...
                    $(".uk-modal-content").html("");
                    show_flash_msg(result['msg'], result['status']);
                    refresh_or_load(grid_id);
                },
            });
        }
    }

    function refresh_or_load(formid) {
//                $.noConflict(true);
        try {
            $.fn.yiiGridView.update(formid);
        }
        catch (e) {
            window.location.href = window.location.href;
        }
    }
    function audilogpopup(event, $this) {
        event.preventDefault();
        var title = $this.data("title");
        $('.uk-modal-title').html(title);
        $.ajax({
