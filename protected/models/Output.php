...
class Output extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, name, status_op', 'required'),
			array('id, name', 'numerical', 'integerOnly'=>true),
			array('status_op', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, status_op', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('status_op',$this->status_op,true);
